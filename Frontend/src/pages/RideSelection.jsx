...
export default function RideSelection() {
  ...
  const [distance, setDistance] = useState(5); // temporary default
  ...
  // 🔥 FAKE DISTANCE (later replace with real)
  useEffect(() => {
    setDistance(4.5);
  }, []);
  ...
  const confirmRide = async () => {
    try {
      ...
      const pickupCoords = await getCoordinates(state.pickup);
      const destinationCoords = await getCoordinates(state.destination);

      if (!pickupCoords || !destinationCoords) {
        return toast.error("Invalid location");
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white p-4">
      ...
      {/* HEADER */}
      <h1 className="text-2xl font-bold mb-3">Choose your ride</h1>
      ...
      <div className="bg-white/10 p-3 rounded-xl mb-4 text-sm">
        ...
        <p className="text-gray-400">→ {state.destination}</p>
      ...
      {Object.entries(rideTypes).map(([key, ride]) => (
        <div
          ...
          className={`flex items-center justify-between p-4 mb-3 rounded-2xl cursor-pointer transition ${
            selected === key
      ...
      <button
        onClick={confirmRide}
        className="w-full mt-4 py-3 rounded-2xl bg-green-500 hover:bg-green-600 text-lg font-semibold"
      >
        {loading
          ? "Booking..."
          : `Confirm ${rideTypes[selected].name}`}
      </button>
