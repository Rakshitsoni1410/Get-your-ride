...
import { DollarSign, Clock, User, LogOut, Car, CheckCircle, Navigation2 } from "lucide-react";
...
export default function CaptainDashboard() {
    ...
    try {
      ...
      if (res.ok && data.ride) {
        ...
        socket.emit("accept-ride", {
          ...
          userId: typeof data.ride.user === "object" ? data.ride.user._id : data.ride.user,
          captainId,
        });

        // Immediately ask for customer location
        const userId = typeof data.ride.user === "object" ? data.ride.user._id : data.ride.user;
  ...
  const rejectRide = () => { setRide(null); toast.info("Ride rejected"); };
  ...
  const completeRide = async () => {
    try {
      ...
      socket.emit("ride-completed-socket", {
        ...
        userId: typeof activeRide.user === "object" ? activeRide.user._id : activeRide.user,
      });
      ...
      setActiveRide(null);
      setCustomerPos(null);
      toast.success("Ride completed! ₹" + (activeRide.fare || 0) + " earned ✅");
    } catch {
      toast.error("Error completing ride");
    }
  };

  const logout = () => { stopGPS(); localStorage.clear(); navigate("/"); };
      ...
      <div className="px-4 pt-10 pb-4">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold" style={{ fontFamily: "Syne, sans-serif" }}>Captain Mode</p>
        ...
        >
          <span className={`w-3 h-3 rounded-full ${isOnline ? "bg-red-400 animate-pulse" : "bg-black"}`} />
      ...
      <div className="flex-1 px-4 pb-8">
        ...
        {!isOnline && !activeRide && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-white font-semibold mb-1" style={{ fontFamily: "Syne, sans-serif" }}>Ready to earn?</p>
        ...
        {isOnline && !ride && !activeRide && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-white font-semibold mb-1" style={{ fontFamily: "Syne, sans-serif" }}>Searching for rides...</p>
        ...
        {ride && (
          <div className="bg-[#111] border-2 border-[#F5C518]/40 rounded-2xl p-5 mb-4">
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              ...
              <div className="bg-[#1a1a1a] rounded-xl p-3 text-center">
                <p className="text-[#F5C518] font-bold text-lg" style={{ fontFamily: "Syne, sans-serif" }}>{ride.distance} km</p>
                <p className="text-[#555] text-xs">Distance</p>
              </div>
              <div className="bg-[#1a1a1a] rounded-xl p-3 text-center">
                <p className="text-[#F5C518] font-bold text-lg capitalize" style={{ fontFamily: "Syne, sans-serif" }}>{ride.vehicleType}</p>
                <p className="text-[#555] text-xs">Vehicle</p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <button onClick={rejectRide} className="py-3.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 font-semibold hover:bg-red-500/20 transition">
                Reject
              </button>
              <button onClick={acceptRide} className="py-3.5 rounded-xl bg-[#F5C518] text-black font-bold hover:bg-yellow-400 active:scale-95 transition" style={{ fontFamily: "Syne, sans-serif" }}>
                Accept 🚗
              </button>
            </div>
          </div>
        ...
        {activeRide && (
          <div>
            ...
            <div className="rounded-2xl overflow-hidden border border-[#1e1e1e] mb-4" style={{ height: "260px" }}>
              <MapContainer
                center={mapCenter}
                zoom={14}
            ...
            <div className="bg-[#111] border border-green-500/25 rounded-2xl p-5 mb-3">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-bold text-green-400 uppercase tracking-widest">Ride Active</p>
                </div>
                <span className="text-[#F5C518] font-mono font-bold" style={{ fontFamily: "Syne, sans-serif" }}>{fmtTime(rideTimer)}</span>
              </div>
              ...
              <div className="flex gap-3 mb-4">
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-[#F5C518] font-bold text-xl" style={{ fontFamily: "Syne, sans-serif" }}>₹{activeRide.fare}</p>
                </div>
              </div>
              ...
              >
                <Navigation2 size={16} /> {customerPos ? "Refresh Customer Location" : "Show Customer on Map"}
              </button>
