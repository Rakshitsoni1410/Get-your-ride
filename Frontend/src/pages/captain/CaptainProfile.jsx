...
import { ArrowLeft, User, Mail, Phone, Car, Hash, Calendar, LogOut, Star, BadgeCheck } from "lucide-react";
...
export default function CaptainProfile() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/captain/me", {
      ...
      .then((r) => r.json())
      .then((d) => { if (d.captain) setCaptain(d.captain); else { toast.error("Unauthorized"); navigate("/"); } })
      .catch(() => { toast.error("Error loading profile"); navigate("/"); });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] px-4 py-10">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-[#666] hover:text-white transition text-sm mb-6">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {captain.isVerified && <BadgeCheck size={20} className="text-blue-400" />}
        </div>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[#F5C518] text-sm flex items-center gap-1"><Star size={13} fill="#F5C518" /> {captain.rating || 5.0}</span>
          <span className="text-[#555] text-sm">· {captain.totalRides || 0} rides</span>
        </div>
        <span className={`mt-2 pill ${captain.status === "online" ? "bg-green-500/15 text-green-400" : "bg-[#1a1a1a] text-[#555]"}`}>
      ...
      <div className="bg-[#111] border border-[#1e1e1e] rounded-2xl px-4 mb-6">
        ...
        <InfoRow icon={User} label="Capacity" value={`${captain.vehicle?.capacity} seats`} />
        {captain.license?.expiry && (
          <InfoRow icon={Calendar} label="License Expiry" value={new Date(captain.license.expiry).toLocaleDateString()} />
        )}
      </div>
